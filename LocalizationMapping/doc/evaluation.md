...
The script launches the existing perception stack and mapping node with `eval_debug:=true`,
plays the configured rosbag, and runs a separate monitor process. Normal runs through
`./scripts/run_dataset_mapping_chain.sh` keep evaluation debug output disabled.
