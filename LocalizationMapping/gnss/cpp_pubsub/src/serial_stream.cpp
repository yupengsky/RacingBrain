...
void SerialStream::check_remove() {
  ...
  if (nsent < 0) {
    std::cout << "Serial stream detect write failed, error: " << strerror(errno) << std::endl;
    switch (errno) {
      case EBADF:
      case EIO:
        status_ = Stream::Status::DISCONNECTED;
        std::cout << "Device " << device_name_ << " removed." << std::endl;
        Disconnect();
        break;
    }
  }
    ...
    if (bytes_current_read < 0) {
      switch (errno) {
        ...
        case EIO:
          ...
          Disconnect();
          if (Connect()) {
            std::cout << "Reconnect " << device_name_ << " success." << std::endl;
            bytes_current_read = 0;
...
size_t SerialStream::write(const uint8_t* data, size_t length) {
  if (!is_open_) {
    ...
    std::cout << "Connect " << device_name_ << " success." << std::endl;
  }
  ...
  while ((length > 0) && (delay_times < 5)) {
    ...
    if (nsent < 0) {
      std::cout << "Serial stream write data failed, error: " << strerror(errno) << std::endl;
      switch (errno) {
        case EAGAIN:
        case EINVAL:
          nsent = 0;
