...
- Models root: `/media/yupeng/新加卷/Models/RacingBrain`
- Default YOLO model: `/media/yupeng/新加卷/Models/RacingBrain/perception/src/cone_ws/src/cone_detector/runs/cone_yolov8n6/weights/best.pt`
...
Dataset-to-mapping integration check:

```bash
./scripts/run_dataset_mapping_chain.sh
```

This launches perception, launches mapping without RViz, replays only the required
...
- Mapping publishes non-empty `/global_map`.
...
- `./scripts/run_dataset_mapping_chain.sh` verifies the runtime path from the
  configured rosbag dataset to non-empty mapping `/global_map` output.
