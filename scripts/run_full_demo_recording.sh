...
OUTPUT_PATH="${OUTPUT_PATH:-${OUTPUT_DIR}/drd26_full_demo_${STAMP}.mp4}"
LOG_DIR="$(mktemp -d "/tmp/drd26_full_demo_${STAMP}_XXXX")"
...
"${WORKSPACE_DIR}/.venv_ros_ml/bin/python" \
  "${WORKSPACE_DIR}/scripts/record_full_demo.py" \
  --output "${OUTPUT_PATH}" \
  --fps "${FPS}" \
...
LOG_DIR="${LOG_DIR}" \
  ...
  RVIZ=true \
  KEEP_RUNNING=true \
  "${WORKSPACE_DIR}/scripts/run_dataset_slam_chain.sh" \
