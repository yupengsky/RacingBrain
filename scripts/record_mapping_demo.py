...
class MappingDemoRecorder(Node):
    def __init__(self, output_path: Path, fps: float, idle_timeout: float, startup_timeout: float) -> None:
        super().__init__("racingbrain_mapping_demo_recorder")
    ...
    def draw_header(self, canvas: np.ndarray, elapsed: float) -> None:
        cv2.putText(canvas, "RacingBrain Mapping Demo", (20, 36), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (35, 35, 35), 2, cv2.LINE_AA)
        cv2.putText(
            canvas,
    ...
    def draw_status_panel(self, panel: np.ndarray, elapsed: float) -> None:
        ...
        if self.latest_vehicle_odom is not None:
            lines.append(f"Mapping odom x/y     : {self.latest_vehicle_odom[0]:8.3f}, {self.latest_vehicle_odom[1]:8.3f}")
        ...
        lines.extend(
            [
                "",
                "Pipeline:",
                "image_raw -> YOLO -> LiDAR segmentation -> fusion -> GNSS/INS mapping",
...
def main() -> None:
    ...
    rclpy.init()
    node = MappingDemoRecorder(output_path, args.fps, args.idle_timeout, args.startup_timeout)
    try:
