...
仓库原有 RacingBrain 规划接口已经能从稳定锥桶图生成稀疏赛道图：
...
此外，`control/src` 已整理出一个独立规划控制层：

- `path_planner`: autocross、acceleration、skidpad 的局部路径规划。
- `cpp_controller`: C++ 横纵向 PID 控制器，带曲率速度剖面。
- `simple_pid_controller`: Python fallback 控制器。
- `racing_control_adapters`: `/global_map`、`/vehicle_odom` 到 `/drd25/map`、`/testing_only/odom` 的接口适配。
- `racing_control_bringup`: 规划控制 launch 与任务参数。

核心入口：

- `LocalizationMapping/RacingBrain/racingbrain/planning/track_graph_node.py`
- `control/src/racing_control_bringup/launch/racing_control.launch.py`
- `control/src/cpp_controller/src/PID_controller_node.cpp`
- `control/src/path_planner/path_planner/planner_node.py`
...
当前已经有导入并清理后的规划控制层，但还没有完成与真实车或 FSDS 的闭环验证。下一步应补：
